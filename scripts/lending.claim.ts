...
async function main(){
    ...
    let loan = await lending.getLoan(Address.parse(itemAddress));
    ...
    await lending.sendClaim(walletContract.sender(keyPair.secretKey), toNano("0.02"), Address.parse(itemAddress));
    await new Promise(f => setTimeout(f, 25 * 1000));
    loanPool = await lending.getLoanPool();
    console.log(`loan pool size: ${loanPool.size}`)
    loan = await lending.getLoan(Address.parse(itemAddress));
