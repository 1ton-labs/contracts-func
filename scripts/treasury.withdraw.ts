...
async function main() {
    ...
    const {owner,balance} = await treasuryPool.getPoolData()
    if (!wallet.address.equals(owner) || !(withdrawAmount>=balance)) throw new Error(`Only owner can withdraw`)
    //TODO: check how much ton coin will be used
    await treasuryPool.sendWithdraw(walletContract.sender(keyPair.secretKey), {
        value: toNano("0.1"),
